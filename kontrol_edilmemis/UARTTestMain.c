...
void bilmem(int *array, int klm){
	...
	for (i = 0; i< 16; i++){
		if(klm == 15){
			...
			numArray[15] = 1; //numArrayde arti("A") isleminin oldugu yer 1 oldu ve en son arti isleminin yapildigi anlasiliyor.
		}
		if(klm == 10){
			if(array[i] == 1){
				carpma = numArray[i]; // carpma("*") degeri numArray(numarraydaki deger) degerine atandi			
			}
			numArray[15] = 1; //numArrayde arti isleminin oldugu yer 1 oldu ve en son arti isleminin yapildigi anlasiliyor.
		}		
		if(klm == 11){
			if(array[i] == 1){
				int esit;
				...
				if(numArray[15] == 1){												//Toplama Islemine giriyor
					...
					UART_OutString(" = ");
					UART_OutUDec(esit);					
				}else if(numArray[10] == 1){										// Carpma Islemine giriyor.
					...
					UART_OutString(" = ");
					UART_OutUDec(esit);	
					numArray[10] = 0;														// Carpma islemi degerini sifirla, yani carpma gorevini tamamladi.
					
				}else{
					UART_OutString("Not adding or multiplication is found! ");
				}
			}
		}		
		if(i != klm){
